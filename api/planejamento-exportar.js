...
const TABELA_POR_TIPO = {
  dfd: 'planejamento_dfd',
  etp: 'planejamento_etp',
  tr: 'planejamento_tr',
  mapa_riscos: 'planejamento_mapa_riscos',
};
...
export default async function handler(req, res) {
  ...
  try {
    const docxBuf = await buildPlanejamentoDocx({ tipoPeca, conteudoGerado, processoObjeto, numeroProcesso });
    const nomeArquivo = nomeArquivoPlanejamento({ tipoPeca, numeroProcesso });
    const path = `pecas/${tipoPeca}/${pecaId}/${nomeArquivo}`;
    ...
    const { data: row, error } = await sb.from(tabela)
      .update({ arquivo_docx_url: docxUrl, status: 'finalizado', updated_at: new Date().toISOString() })
      .eq('id', pecaId)
      .select()
      .single();
